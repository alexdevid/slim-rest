...
abstract class Token implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Models\\Map\\TokenTableMap';
    ...
    /**
     * Initializes internal state of Models\Base\Token object.
     */
     ...
     * @return $this|\Models\Token The current object (for fluent API support)
     */
    ...
    public function setClientId($v)
    {
        ...
        if ($this->client_id !== $v) {
            $this->client_id = $v;
            $this->modifiedColumns[TokenTableMap::COL_CLIENT_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : TokenTableMap::translateFieldName('ClientId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : TokenTableMap::translateFieldName('Scope', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Models\\Token'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(TokenTableMap::COL_CLIENT_ID)) {
            $modifiedColumns[':p' . $index++]  = 'client_id';
        }
        if ($this->isColumnModified(TokenTableMap::COL_TOKEN)) {
            $modifiedColumns[':p' . $index++]  = 'token';
        }
        if ($this->isColumnModified(TokenTableMap::COL_SCOPE)) {
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Models\Token The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(TokenTableMap::COL_CLIENT_ID)) {
            $criteria->add(TokenTableMap::COL_CLIENT_ID, $this->client_id);
        }
        if ($this->isColumnModified(TokenTableMap::COL_TOKEN)) {
            $criteria->add(TokenTableMap::COL_TOKEN, $this->token);
        }
        if ($this->isColumnModified(TokenTableMap::COL_SCOPE)) {
            $criteria->add(TokenTableMap::COL_SCOPE, $this->scope);
        }
        if ($this->isColumnModified(TokenTableMap::COL_EXPIRES)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Models\Token (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Models\Token Clone of current object.
     * @throws PropelException
     */
