...
abstract class Client implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Models\\Map\\ClientTableMap';
    ...
    /**
     * @var        ObjectCollection|ChildCode[] Collection to store aggregation of ChildCode objects.
     */
    protected $collCodes;
    ...
    /**
     * An array of objects scheduled for deletion.
     * @var ObjectCollection|ChildCode[]
     */
    protected $codesScheduledForDeletion = null;

    /**
     * Initializes internal state of Models\Base\Client object.
     */
     ...
     * @return $this|\Models\Client The current object (for fluent API support)
     */
    ...
    public function setClientId($v)
    {
        ...
        if ($this->client_id !== $v) {
            $this->client_id = $v;
            $this->modifiedColumns[ClientTableMap::COL_CLIENT_ID] = true;
        }
    ...
    public function setClientSecret($v)
    {
        ...
        if ($this->client_secret !== $v) {
            ...
            $this->modifiedColumns[ClientTableMap::COL_CLIENT_SECRET] = true;
        }
    ...
    public function setGrantTypes($v)
    {
        ...
        if ($this->grant_types !== $v) {
            $this->grant_types = $v;
            $this->modifiedColumns[ClientTableMap::COL_GRANT_TYPES] = true;
        }
    ...
    public function setRedirectUri($v)
    {
        ...
        if ($this->redirect_uri !== $v) {
            $this->redirect_uri = $v;
            $this->modifiedColumns[ClientTableMap::COL_REDIRECT_URI] = true;
        }
    ...
    public function setScope($v)
    {
        ...
        if ($this->scope !== $v) {
            $this->scope = $v;
            $this->modifiedColumns[ClientTableMap::COL_SCOPE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : ClientTableMap::translateFieldName('ClientId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->client_id = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : ClientTableMap::translateFieldName('ClientSecret', TableMap::TYPE_PHPNAME, $indexType)];
            $this->client_secret = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : ClientTableMap::translateFieldName('GrantTypes', TableMap::TYPE_PHPNAME, $indexType)];
            $this->grant_types = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : ClientTableMap::translateFieldName('RedirectUri', TableMap::TYPE_PHPNAME, $indexType)];
            $this->redirect_uri = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : ClientTableMap::translateFieldName('Scope', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Models\\Client'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ClientTableMap::COL_CLIENT_ID)) {
            $modifiedColumns[':p' . $index++]  = 'client_id';
        }
        if ($this->isColumnModified(ClientTableMap::COL_CLIENT_SECRET)) {
            $modifiedColumns[':p' . $index++]  = 'client_secret';
        }
        if ($this->isColumnModified(ClientTableMap::COL_GRANT_TYPES)) {
            $modifiedColumns[':p' . $index++]  = 'grant_types';
        }
        if ($this->isColumnModified(ClientTableMap::COL_REDIRECT_URI)) {
            $modifiedColumns[':p' . $index++]  = 'redirect_uri';
        }
        if ($this->isColumnModified(ClientTableMap::COL_SCOPE)) {
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collCodes) {
                ...
                $result[$key] = $this->collCodes->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Models\Client The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ClientTableMap::COL_CLIENT_ID)) {
            $criteria->add(ClientTableMap::COL_CLIENT_ID, $this->client_id);
        }
        if ($this->isColumnModified(ClientTableMap::COL_CLIENT_SECRET)) {
            $criteria->add(ClientTableMap::COL_CLIENT_SECRET, $this->client_secret);
        }
        if ($this->isColumnModified(ClientTableMap::COL_GRANT_TYPES)) {
            $criteria->add(ClientTableMap::COL_GRANT_TYPES, $this->grant_types);
        }
        if ($this->isColumnModified(ClientTableMap::COL_REDIRECT_URI)) {
            $criteria->add(ClientTableMap::COL_REDIRECT_URI, $this->redirect_uri);
        }
        if ($this->isColumnModified(ClientTableMap::COL_SCOPE)) {
            $criteria->add(ClientTableMap::COL_SCOPE, $this->scope);
        }
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Models\Client Clone of current object.
     * @throws PropelException
     */
    ...
    public function initTokens($overrideExisting = true)
    {
        ...
        $this->collTokens->setModel('\Models\Token');
    }
    ...
    public function initCodes($overrideExisting = true)
    {
        ...
        $this->collCodes = new ObjectCollection();
        $this->collCodes->setModel('\Models\Code');
    }

    /**
     * Gets an array of ChildCode objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     * @return ObjectCollection|ChildCode[] List of ChildCode objects
     * @throws PropelException
     */
    public function getCodes(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        ...
        if (null === $this->collCodes || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collCodes) {
                ...
                $collCodes = ChildCodeQuery::create(null, $criteria)
                    ->filterByClient($this)
                    ->find($con);
    ...
    /**
     * Sets a collection of ChildCode objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $codes A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildClient The current object (for fluent API support)
     */
    public function setCodes(Collection $codes, ConnectionInterface $con = null)
    {
        /** @var ChildCode[] $codesToDelete */
     ...
     * @throws PropelException
     */
    public function countCodes(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collCodesPartial && !$this->isNew();
        if (null === $this->collCodes || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collCodes) {
            ...
            $query = ChildCodeQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
    ...
    /**
     * Method called to associate a ChildCode object to this object
     * through the ChildCode foreign key attribute.
     *
     * @param  ChildCode $l ChildCode
     * @return $this|\Models\Client The current object (for fluent API support)
     */
    public function addCode(ChildCode $l)
    {
        if ($this->collCodes === null) {
            $this->initCodes();
            $this->collCodesPartial = true;
    ...
    /**
     * @param  ChildCode $code The ChildCode object to remove.
     * @return $this|ChildClient The current object (for fluent API support)
     */
    public function removeCode(ChildCode $code)
    {
        if ($this->getCodes()->contains($code)) {
            $pos = $this->collCodes->search($code);
            $this->collCodes->remove($pos);
